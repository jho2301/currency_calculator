  ...
  <div class="layout">
    ...
    <transition-group name="list">
      <div v-for="arrNum of max" :key="arrNum" class="input-row">
        ...
        <input
          type="text"
          class="foreign-currency-input"
          v-model="insertedPrice[arrNum-1]"
          @input="setCost(arrNum, quantity[arrNum-1], insertedPrice[arrNum-1],'setProductPrice')"
        />
        <input
          type="number"
          v-model="quantity[arrNum-1]"
          @input="setCost(arrNum, quantity[arrNum-1], insertedPrice[arrNum-1],'setProductPrice')"
          value="1"
          min="0"
          max="99"
...
export default {
  ...
  methods: {
    ...
    setCostKrw(arrNum) {
      let data = Math.floor(
        this.$store.state.productPrice[arrNum - 1] *
...
.add-btn {
  ...
  box-shadow: 0 3px 5px rgba(20, 8, 41, 0.274),
    inset 0 2px 2px rgba(255, 246, 246, 0.226),
    inset 0 -2px 2px rgba(67, 59, 68, 0.226);
  font-weight: 600;
  text-align: center;
  align-self: center;
  width: 76%;
